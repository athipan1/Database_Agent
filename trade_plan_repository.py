...
from models import CreateTradePlanBody, TradePlanRecord, UpdateTradePlanStatusBody
...
def setup_trade_plan_table(db) -> None:
...
def create_trade_plan(db, body: CreateTradePlanBody) -> TradePlanRecord:
    setup_trade_plan_table(db)
    ...
    with db.connection_scope() as conn:
        ...
        try:
            ...
            conn.commit()
            record = get_trade_plan(db, body.trade_plan_id)
            if not record:
...
def get_trade_plan(db, trade_plan_id: str) -> Optional[TradePlanRecord]:
    setup_trade_plan_table(db)
    with db.connection_scope() as conn:
        cursor = db.get_cursor(conn)
        try:
...
def update_trade_plan_status(db, trade_plan_id: str, body: UpdateTradePlanStatusBody) -> TradePlanRecord:
    setup_trade_plan_table(db)
    now = _now_iso()
    with db.connection_scope() as conn:
        cursor = db.get_cursor(conn)
        try:
            ...
            conn.commit()
            updated = get_trade_plan(db, trade_plan_id)
            if not updated:
